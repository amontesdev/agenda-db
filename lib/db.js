...
export function getDb(useProduction = false) {
  console.log("[getDb] useProduction:", useProduction, "TURSO_DATABASE_URL:", !!process.env.TURSO_DATABASE_URL);
  if (useProduction && process.env.TURSO_DATABASE_URL) {
    console.log("[getDb] returning prodClient");
    return getProdClient();
  }
  console.log("[getDb] returning localClient");
