...
const clonePreset = (preset) => Object.fromEntries(
  Object.entries(preset).map(([opt, blocks]) => [opt, blocks.map((block) => ({ ...block }))])
);

export const PRESETS = {
  semana: clonePreset(WEEKDAY_PRESETS),
  sabado: {
...
export const WEEKDAY_KEYS = ["lunes","martes","miercoles","jueves","viernes"];
WEEKDAY_KEYS.forEach((day) => {
  PRESETS[day] = clonePreset(WEEKDAY_PRESETS);
});

export const WEEKEND_KEYS = ["sabado","domingo"];
...
export const DAY_KEYS = ["semana", ...WEEKDAY_KEYS, "sabado", "domingo"];
